package com.zseltzer.superheroes.proxies;
...
public class ClientProxy extends CommonProxy
{
    ...
    private void addArmorModel(Item armor, ModelBiped model)
    {
        if (model == null)
        {
            ...
            Main.logger.log(Level.WARNING, String.format("Error adding model for %s: Cannot add a NULL armor model!", armor.getUnlocalizedName()));
            return;
        }
        // better let yourself / users know when overwriting an entry, as it is likely to be a mistake!
        if (armorModels.containsKey(armor))
        {
            Main.logger.log(Level.WARNING, String.format("A model for %s has already been registered! It will now be overwritten.", armor.getUnlocalizedName()));
        }
